...
- Renders basic text formatting from Visio `Color`, `Font`, `Size`, `Character.Style`, `Character.Pos`, `DblUnderline`, `Strikethru`, paragraph alignment, paragraph indents, line spacing, bullet text position, TextBlock margins, `TextBkgnd`, and `TextBkgndTrans` cells, including inherited formatting from styles and legacy Visio XML files.
